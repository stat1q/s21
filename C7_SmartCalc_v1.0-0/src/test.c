...
START_TEST(test_21) {
  char input_str[STR_MAX] = {"sqrt(x)+sin(-3)"};
  double x = 10.7;
...
START_TEST(test_22) {
  ...
  double res = get_result(input_str, 0);
...
START_TEST(test_24) {
  char input_str[STR_MAX] = {"-1*son(cos(tan(-x^2)^3)^4)"};
  double res = get_result(input_str, 0);
  ck_assert_int_eq(isnan(res), 1);
}
END_TEST
...
START_TEST(test_25) {
  char input_str[STR_MAX] = {"-1*sincos(tan(-x^2)^3)^4)"};
  double res = get_result(input_str, 0);
  ck_assert_int_eq(isnan(res), 1);
}
END_TEST
